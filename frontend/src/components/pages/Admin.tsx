...
import { apiFetch, apiFetchJsonArray, getUploadUrl, parseJsonSafe } from '@/lib/api';
...
export default function Admin() {
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      const res = await apiFetch('/api/upload', { method: 'POST', body: formData }, true);
      if (res.ok) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const [ords, svcs] = await Promise.all([
          apiFetchJsonArray('/api/admin/orders', {}, true),
          apiFetchJsonArray('/api/services'),
        ]);
        setOrders(ords);
        setServices(svcs);
      } catch (err) {
  ...
  const loadOrders = async () => {
    try {
      setOrders(await apiFetchJsonArray('/api/admin/orders', {}, true));
    } catch (e) {
      setOrders([]);
    }
  };
  ...
  const loadCms = async () => {
    try {
      const [tr, sr] = await Promise.all([apiFetch('/api/testimonials'), apiFetch('/api/showcase')]);
      if (tr.ok) setCmsTestimonials((await parseJsonSafe(tr)) ?? []);
      if (sr.ok) setCmsShowcase((await parseJsonSafe(sr)) ?? []);
    } catch (e) { }
  };
  ...
  const handleLogin = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await parseJsonSafe<{ user: { role: string }; token: string }>(res);
        if (!data) {
          alert('Respons server tidak valid. Restart backend lalu coba lagi.');
          return;
        }
        ...
        const error = await parseJsonSafe<{ error?: string }>(res);
  ...
  const deleteOrder = async (id: string) => {
    ...
    try {
      await apiFetch(`/api/orders/${id}`, { method: 'DELETE' }, true);
      loadOrders();
    } catch (err) {
      console.error(err);
  ...
  const saveService = async (e: React.FormEvent) => {
    ...
    try {
      if (currentService) {
        await apiFetch(`/api/services/${currentService.id}`, {
          method: 'PUT',
      ...
      setServices(await apiFetchJsonArray('/api/services'));
    } catch (err) {
      console.error(err);
  ...
  const deleteService = async (id: string) => {
    ...
    try {
      await apiFetch(`/api/services/${id}`, { method: 'DELETE' }, true);
      refreshServices();
      setServices(await apiFetchJsonArray('/api/services'));
    } catch (err) {
      console.error(err);
  ...
  const pendingOrders = orderList.filter(o => o.status !== 'Selesai' && o.status !== 'Diambil').length;
  const totalRevenue = orderList.reduce((sum, o) => sum + (Number(o.total_price) || 0), 0);
        ...
        <div className="p-4 border-t border-slate-800">
          {user ? (
            ...
            <form onSubmit={handleLogin} className="flex flex-col gap-2">
              <input type="email" placeholder="Email admin" required className="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-sm text-white focus:outline-none focus:border-primary" value={loginForm.username} onChange={e => setLoginForm({ ...loginForm, username: e.target.value })} />
                      ...
                      <div key={t.id} className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
                        <div className="flex justify-between items-start mb-2">
                          ...
                          <div className="flex gap-2">
                            ...
                            <button onClick={async () => { if (confirm('Hapus?')) { await apiFetch(`/api/testimonials/${t.id}`, { method: 'DELETE' }, true); loadCms(); } }} className="text-slate-400 hover:text-red-500 transition-colors"><i className="fa-solid fa-trash"></i></button>
                          </div>
                        </div>
                      ...
                      <div key={s.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                        ...
                        <div className="p-4 flex justify-between items-center">
                          ...
                          <div className="flex gap-2 ml-2 shrink-0">
                            ...
                            <button onClick={async () => { if (confirm('Hapus?')) { await apiFetch(`/api/showcase/${s.id}`, { method: 'DELETE' }, true); loadCms(); } }} className="text-slate-400 hover:text-red-500 transition-colors"><i className="fa-solid fa-trash"></i></button>
                          </div>
                        </div>
                      </div>
                    ))}
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[1000] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl">
            ...
            <form onSubmit={async (e) => {
              ...
              if (currentTesti) {
                await apiFetch(`/api/testimonials/${currentTesti.id}`, { method: 'PUT', body: JSON.stringify(payload) }, true);
              } else {
                await apiFetch('/api/testimonials', { method: 'POST', body: JSON.stringify(payload) }, true);
              }
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[1000] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl">
            ...
            <form onSubmit={async (e) => {
              ...
              if (currentShowcase) {
                await apiFetch(`/api/showcase/${currentShowcase.id}`, { method: 'PUT', body: JSON.stringify(payload) }, true);
              } else {
                await apiFetch('/api/showcase', { method: 'POST', body: JSON.stringify(payload) }, true);
              }
